...
export class AudioManager {
  ...
  unlock(){
    ...
    if(!this.context||this.context.state==='closed')this.context=new Ctx();
    this.synth.context=this.context;this.motor.context=this.context;
    this.synth.ensure();this.motor.ensure();
    return this.context.state!=='running'?this.context.resume().then(()=>true).catch(()=>false):Promise.resolve(true);
  }
