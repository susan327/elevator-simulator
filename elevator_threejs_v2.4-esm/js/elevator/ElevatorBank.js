...
export class ElevatorBank {
  constructor(scene,building,geometryConfig,{config,audio,log,shouldPlayArrival}){
    ...
    for(const id of building.shaftIds){const car=new ElevatorCar(scene,building,geometryConfig,{id,shaftX:building.getShaftCenter(id)}),doors=new DoorController(car,building,id),service={isServed:floor=>config.isServed(floor,id)},calls=new CallController(service),controller=new ElevatorController(car,doors,calls,building,text=>log(`${id}号機：${text}`),config,audio,floor=>shouldPlayArrival?.(id,floor)??true);this.units.set(id,{id,car,doors,calls,controller});}
