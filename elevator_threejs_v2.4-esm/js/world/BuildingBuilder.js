...
export class BuildingBuilder {
  ...
  serviceFloorLabel(id){const floors=this.geometryConfig?.system?.servedFloorsFor?.(id)||[];if(!floors.length)return '停止なし';if(floors.length===this.floors)return '各階';const parts=[];for(let i=0;i<floors.length;){let j=i;while(j+1<floors.length&&floors[j+1]===floors[j]+1)j++;const count=j-i+1;parts.push(count>=3?`${floors[i]}〜${floors[j]}`:floors.slice(i,j+1).join('・'));i=j+1;}return `${parts.join('・')}F`;}
  ...
  buildInteriorLayout(lobby,f,profile,lobbyH){const layout=FloorLayoutCatalog.get(this.geometryConfig?.system?.buildingPreset||'custom',f,profile);lobby.userData.floorLayout=layout;this.interiorDoors[f]={};for(const blocker of layout.blockers){const b=blocker.bounds,w=b.maxX-b.minX,d=b.maxZ-b.minZ;this.box(w,2.28,d,profile.wall,(b.minX+b.maxX)/2,1.14,(b.minZ+b.maxZ)/2,{roughness:.91},lobby);}for(const portal of layout.portals.filter(x=>x.kind==='door')){this.box(1.06,.10,.16,profile.trim,portal.x,2.24,portal.z,{metalness:.12,roughness:.58},lobby);const door=this.box(.94,2.08,.075,profile.accent,portal.x,1.04,portal.z-.02,{metalness:.18,roughness:.48},lobby);this.interiorDoors[f][portal.target]=door;this.sign(portal.target==='left-room'?'01':'02',.42,.18,lobby,portal.x,2.05,portal.z-.09,'#182027','#f3d99b');}this.box(10.7,.035,.08,profile.trim,0,.025,4.35,{roughness:.72},lobby);this.box(10.7,.035,.08,profile.accent,0,.027,7.12,{roughness:.72},lobby);}
  build(){
  ...
  buildFloor(f){
    ...
    for(const id of this.shaftIds){const doorX=this.shaftCenters[id],doorGroup=new THREE.Group();doorGroup.position.set(doorX,0,c.hallDoorZ);lobby.add(doorGroup);const left=this.makeDoorLeaf(doorGroup,'left'),right=this.makeDoorLeaf(doorGroup,'right');this.hallDoors[id][f]={group:doorGroup,left,right};this.shaftBacks[id][f]=this.shaftCurtain(c.doorPanelTotalWidth,c.doorHeight,doorX,c.doorHeight/2,c.cabinDoorZ-.12,lobby);this.setHallDoorOpen(f,0,id);this.box(1.38,.42,.10,0x0d0f12,doorX,openingH+.29,1.62,{metalness:.45,roughness:.23},lobby);const indicator=this.hallIndicators[id][f]=this.makeTravelIndicator(lobby,doorX,openingH+.29,1.676,1.20,.29),unitLabel=this.sign(`${id}号機`,1.06,.28,lobby,doorX,openingH+.62,1.676,'#182027','#f5db83',82),serviceLabel=this.sign(`停車 ${this.serviceFloorLabel(id)}`,1.72,.27,lobby,doorX,openingH+.91,1.676,'#101820','#e7f2f8',70);for(const target of [indicator,unitLabel,serviceLabel]){target.userData.interaction={type:'hallFocus',elevatorId:id};this.interactiveObjects.push(target);}}
  ...
  setInteriorDoorOpen(floor,target,open){const door=this.interiorDoors[floor]?.[target];if(!door)return;door.visible=!open;}
