...
export class BuildingBuilder {
  ...
  makeDoorLeaf(parent,side,z=0){
    ...
    const glass=this.box(Math.max(.06,visibleWindowWidth-.025),Math.max(.12,windowTop-windowBottom-.095),.028,0xb7c3c7,0,(windowBottom+windowTop)/2,-.055,{transparent:true,opacity:.21,roughness:.08,metalness:.02,side:THREE.DoubleSide},leaf);
    glass.material=new THREE.MeshPhysicalMaterial({color:0xb7c3c7,transparent:true,opacity:.21,roughness:.08,metalness:.02,transmission:.10,side:THREE.DoubleSide});
  ...
  buildFloorScenery(lobby,f,lobbyH){
    const backZ=9.95;
    let type='office',accent=0x53636b;
    if(f===1){type='lobby';accent=0x8a6848;}
    else if(f===this.floors){type='observation';accent=0x315f78;}
    else if(f>=Math.max(2,this.floors-2)){type='executive';accent=0x5c4b63;}
    else if(f%10===6){type='cafe';accent=0x7b5a43;}
    else if(f%10===1){type='lounge';accent=0x617256;}
    else if(f%5===0){type='meeting';accent=0x4d6274;}
    else accent=[0x53636b,0x746858,0x65705b,0x705e67][f%4];
  ...
  buildFloor(f){
    const y=this.floorY(f),c=this.geometryConfig,lobby=new THREE.Group();lobby.position.y=y;this.group.add(lobby);const wall=0xd5cabc,trim=0x75695c,openingW=c.frameOuterWidth,openingH=c.frameOuterHeight,lobbyH=this.floorHeight;
