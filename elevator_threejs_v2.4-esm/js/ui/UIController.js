export class UIController {
  ...
  bind(){
    ...
    const input=this.el('servedFloorsInput');if(input){const applyServiceInput=()=>{if(this.app.config.applyServiceExpression(input.value)){input.classList.remove('input-error');this.updateDesignValues();this.updateValidation();this.app.scheduleDesignUpdate('building');}else{input.classList.add('input-error');this.updateValidation();this.app.log(this.app.config.floorService.lastError||'停止階設定を解釈できません');}};input.addEventListener('blur',applyServiceInput);input.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();input.blur();}});}
  }
