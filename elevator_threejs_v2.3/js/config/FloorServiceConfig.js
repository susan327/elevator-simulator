class FloorServiceConfig {
  ...
  parseExpression(text){
    const set=new Set();
    String(text||'').split(',').map(x=>x.trim()).filter(Boolean).forEach(token=>{
      const range=token.match(/^(\d+)\s*[-~]\s*(\d+)$/);
      if(range){let a=Number(range[1]),b=Number(range[2]);if(a>b)[a,b]=[b,a];for(let n=a;n<=b;n++)if(n>=1&&n<=this.floorCount)set.add(n);}
      else{const n=Number(token);if(Number.isInteger(n)&&n>=1&&n<=this.floorCount)set.add(n);}
    });
    set.add(1);set.add(this.floorCount);return [...set].sort((a,b)=>a-b);
  }
  applyExpression(text,silent=false){
    const served=this.parseExpression(text);if(served.length<2)return false;
