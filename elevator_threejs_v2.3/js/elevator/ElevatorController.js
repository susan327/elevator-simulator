class ElevatorController {
  constructor(car,doors,calls,building,log,motionConfig,audio){
    ...
    this.planStartFloor=1;this.planDirection=0;this.planner=new MotionPlanner(motionConfig);this.currentRequestDirection='car';this.arrivalSoundPlayed=false;this.arrivalLeadSeconds=4;this.sameFloorTimer=0;this.sameFloorArrivalPlayed=false;this.sameFloorDirection=0;this.sameFloorOpenDelay=.80;this.sameFloorArrivalDelay=.80;
  ...
  createPlan(target){
    ...
    this.log(`運転曲線生成：最高${this.plan.peakSpeed.toFixed(2)}m/s・加速${accelDistance}m・減速${decelDistance}m`);
  }
  update(dt){
    ...
    if(this.state==='SAME_FLOOR_RESPONSE'){
      ...
      if(this.sameFloorTimer>=this.sameFloorOpenDelay){
        ...
        this.doors.open(this.position);this.state='DOOR';this.log('同一階応答：0.8秒でドア開開始');
      }
      this.sync();return;
    }
    if(this.state==='ARRIVAL_WAIT'){
      this.arrivalWait-=dt;if(this.arrivalWait<=0){this.doors.open(this.position);this.state='DOOR';this.log('ドア開開始');}
      this.sync();return;
    }
    if(this.state==='DOOR'){
      if(this.doors.isOpen()&&!this.boardingLogged){this.boardingLogged=true;this.log('ドア全開・乗降できます');}
      if(this.doors.isClosed())this.state='IDLE';this.sync();return;
    }
    if(!this.doors.isClosed()){this.state='DOOR';this.sync();return;}
    if(this.target===null){
      const request=this.calls.next(this.position);
      this.target=request?.floor??null;
      this.currentRequestDirection=request?.direction??'car';
      if(this.target!==null)this.createPlan(this.target);
    }
    if(this.target===null){
