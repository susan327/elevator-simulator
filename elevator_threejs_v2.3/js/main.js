class ElevatorApp {
  constructor(){
    ...
    document.getElementById('loading').remove();this.log('v2.3起動。可変停止階・3D実機ボタン対応');requestAnimationFrame(t=>this.loop(t));
  }
  createSimulation(){
  ...
  enter(){this.audio.unlock();if(!this.canEnter()){this.log('まだ乗車できません。停止とドア開を待ってください');return;}this.inside=true;this.camera.setCabin();this.boardingCloseTimer=1.0;this.log(`${this.playerFloor}Fから乗車`);}
  ...
  openDoor(){if(this.inside&&Math.abs(this.elevator.velocity)<.035&&Math.abs(this.elevator.position-Math.round(this.elevator.position))<.08){this.boardingCloseTimer=null;this.doors.open(Math.round(this.elevator.position));this.elevator.state='DOOR';this.log('開ボタン');}}
  closeDoor(){if(this.inside){this.boardingCloseTimer=null;this.doors.close();this.elevator.state='DOOR';this.log('閉ボタン');}}
  ...
  loop(now){
    ...
    if(this.boardingCloseTimer!==null){this.boardingCloseTimer-=dt.sim;if(this.boardingCloseTimer<=0){this.boardingCloseTimer=null;this.doors.close();this.elevator.state='DOOR';this.log('乗車後、自動閉扉開始');}}
